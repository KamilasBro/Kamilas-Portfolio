...
import coreCords from "../../../data/techStack/old/coreCords.json";
      ...
      <div className={`pcb ${!inView && "not-in-view"}`} ref={pcbRef}>
        <ul className="pcb-components">
          {techStack.map((comp, idx) => (
            ...
            >
              {comp.stack.map((node, idy) => (
                ...
                <ul key={idy} className={`nodes ${node.nodeName.toLowerCase()}`}>
                  <li
                    ...
                    onClick={() => { handleNodeClick(comp.sectionName, node.nodeName) }}
                  >
                  ...
                  {node.subnodes?.map((subnode, idz) => (
                    <li
                      ...
                      onClick={() => { handleNodeClick(comp.sectionName, node.nodeName, subnode.nodeName) }}
                    >
              ...
              <li className={`pcb-section`} data-node={comp.sectionName}>{comp.sectionName}</li>
            </ul>
          ))}
        </ul>
        ...
        <div className="paths">
          <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%">
            {techStack.map((comp, idx) => {
              const compXY = getNodeXY(comp.sectionName);
              ...
              return (
                <React.Fragment key={`core-section path ${idx}`}>
                  ...
                  <path
                    data-node={`path core-${comp.sectionName}`}
                    d={`
                  ...
                  {comp.stack
                    .slice()
                    .sort((a: any, b: any) => a.order - b.order) // remove any later
                    .map((node, idy, sorted) => {
                      ...
                      return (
                        <React.Fragment key={`section-mainnode path ${idy}`}>
                          <path
                            data-node={`path ${comp.sectionName}-${node.nodeName}`}
                            d={idx !== 2 ?
                              `
