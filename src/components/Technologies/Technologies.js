...
export default function Technologies() {
    ...
    function returnGraph(ele, index, background, boxShadow){
        return(
            ...
            */}
            {currentTech==="html"?
                htmlSkills.map((ele,index)=>{
                    return returnGraph(
                        ele,
                        index, 
                        "linear-gradient(90deg, #E34C26 0%, #F06529 100%)", 
                        "0px 0px 30px 4px rgba(227, 76, 38, 0.25)"
                        )
                })
            :currentTech==="css"?
                cssSkills.map((ele,index)=>{
                    return returnGraph(
                        ele,
                        index, 
                        "linear-gradient(90deg, #2965f1 0%, #264de4 100%)", 
                        "0px 0px 30px 4px rgba(41, 101, 241, 0.25)"
                        )
            })
            :currentTech==="js"?
                jsSkills.map((ele,index)=>{
                    return returnGraph(
                        ele,
                        index, 
                        "linear-gradient(180deg, #F7DF1E 0%, #F7DF1E 100%)", 
                        "0px 0px 30px 4px rgba(247, 223, 30, 0.25)"
                        )
            })
            :reactSkills.map((ele,index)=>{
                return returnGraph(
                    ele,
                    index, 
                    "linear-gradient(90deg, #88DDED 0%, #7CC5D9 100%)", 
                    "0px 0px 30px 4px rgba(136, 221, 237, 0.25)"
                    )
        })}
        </section>
    )
